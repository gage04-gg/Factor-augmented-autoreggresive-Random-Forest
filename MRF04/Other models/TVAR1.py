...
for target_name, col in TARGET_MAP.items():
    ...
    for h in HORIZONS:
        ...
        preds_TVAR, preds_AR, actuals, dates = [], [], [], []
        ...
        for t in range(len(y_test) - h):
            ...
            Y_roll = df_roll['y']
            X_roll = sm.add_constant(df_roll[[f'y_lag{i}' for i in range(1,p+1)]])

            model_TVAR = sm.OLS(Y_roll, X_roll).fit()
            ...
            for _ in range(h):
                ...
                fcast = model_TVAR.predict(X_last).iloc[0]
        ...
        preds_TVAR = np.array(preds_TVAR)
        preds_AR = np.array(preds_AR)
        actuals = np.array(actuals)

        rmse = np.sqrt(mean_squared_error(actuals, preds_TVAR))

      
        e_model = actuals - preds_TVAR
        ...
        plt.plot(dates, preds_TVAR, linestyle="--", label="TV-AR")
...
summary.to_csv(f"{output_dir}/TVAR_summary.csv", index=False)

print("\n TV-AR MODEL COMPLETE")