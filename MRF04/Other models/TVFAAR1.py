...
for target_name, col in TARGET_MAP.items():
    ...
    print(f"\nRunning TV-FAAR for {target_name}")
    ...
    for h in HORIZONS:
        ...
        preds_TVFAAR, preds_AR, actuals, dates = [], [], [], []
        ...
        for t in range(len(y_test) - h):
            ...
            df_roll = df_reg.iloc[-WINDOW:]

            Y_roll = df_roll['y']
            X_roll = sm.add_constant(df_roll.drop(columns=['y']))

            model_TVFAAR = sm.OLS(Y_roll, X_roll).fit()

          
            Y = df_reg['y']
            X_ar = sm.add_constant(df_reg[[f'y_lag{i}' for i in range(1,p+1)]])
            ...
            for _ in range(h):
                ...
                fcast = model_TVFAAR.predict(X_last).iloc[0]
        ...
        preds_TVFAAR = np.array(preds_TVFAAR)
        preds_AR = np.array(preds_AR)
        actuals = np.array(actuals)

        rmse = np.sqrt(mean_squared_error(actuals, preds_TVFAAR))

        # DM TEST
        e_model = actuals - preds_TVFAAR
        ...
        plt.plot(dates, preds_TVFAAR, linestyle="--", label="TV-FAAR")
...
summary.to_csv(f"{output_dir}/TVFAAR_summary.csv", index=False)

print("\n TV-FAAR MODEL COMPLETE")