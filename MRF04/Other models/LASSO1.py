...
output_dir = "LASSO_FINAL_RESULTS"
...
for target_name, col in TARGET_MAP.items():
    ...
    for h in HORIZONS:
        ...
        preds_LASSO, preds_AR, actuals, dates = [], [], [], []
        ...
        for t in range(len(y_test) - h):
            ...
            for _ in range(h):
                ...
                fcast = model_lasso.predict(X_last_scaled)[0]
        ...
        rmse = np.sqrt(mean_squared_error(actuals, preds_LASSO))

       
        e_model = actuals - preds_LASSO
        ...
        plt.plot(dates, preds_LASSO, linestyle="--", label="LASSO")
...
summary.to_csv(f"{output_dir}/LASSO_summary.csv", index=False)

print("\nLASSO MODEL COMPLETE")