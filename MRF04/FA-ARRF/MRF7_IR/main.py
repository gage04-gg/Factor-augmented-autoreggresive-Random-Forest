...
print("Building S_t ...")
S_df, S_col_names, priority_col_idxs = build_St(data, target="CPIAUCSL", n_y_lags=8)
...
for h in HORIZONS:
    ...
    X, y, S, flags, dates_X = create_X_y_S(data, S_df, target="CPIAUCSL", h=h)
    oos_mask = flags == "oos"
    print(f"  X: {X.shape}  K={X.shape[1]}  |  OOS: {oos_mask.sum()}  "
          f"|  {dates_X[oos_mask][0].date()} → {dates_X[oos_mask][-1].date()}")
    ar_p, ar_a = ar4_benchmark(data, flags, target="CPIAUCSL", h=h)
...
for ax, h in zip(axes, HORIZONS):
    r = results[h]
    act_plot  = r["actuals"]*100
    pred_plot = r["preds"]*100  
...
axes[0].set_ylabel("CPI(T))", fontsize=10)
axes[0].legend(loc="upper left", fontsize=8)
fig.suptitle(
    f"FA-ARRF CPI forecasts vs Realized  ",
    fontsize=12, y=1.02,
)
fig.tight_layout()
